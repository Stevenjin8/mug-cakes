...
%Similarly, given a finite collection $(f(\mathbf{x}_1), \dots, f(\mathbf{x}_n))$, Definition~\ref{def:gp} merely defines a function
%\begin{equation*}
%    \mu_{\mathbf{x}_1, \dots, \mathbf{x}_n}(E) = \int \dots
%\end{equation*}
%that measures how likely $(f(\mathbf{x}_1), \dots, f(\mathbf{x}_n))$ is in a Borel set $E$.
...
In Figure \ref{fig:gp-sample}, we use a kernel such which bases the similarity of $\mathbf{x}$-values on their Euclidean distance.
